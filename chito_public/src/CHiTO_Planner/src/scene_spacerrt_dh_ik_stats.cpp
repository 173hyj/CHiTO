...
static Marker MakeCube(const std::string& frame, const std::string& ns, int id,
                       ...
                       double r, double g, double b, double a=0.9) {
  Marker m;
  m.header.frame_id = frame;
  m.ns = ns; m.id = id;
  m.type = Marker::CUBE;
...
static bool PointInH(const HPolyhedron& H, const Eigen::Vector3d& x, double tol=1e-9) {
...
// Accurate 3D convex polytope volume from H-representation using face reconstruction.
...
static bool ParseSceneFile(const std::string& path, SceneData& out, rclcpp::Logger logger) {
  ...
  for (size_t i=0; i<lines.size();) {
    ...
    std::string type;
    while (j < lines.size() && lines[j].empty()==false && lines[j][0] != '*') {
...
static bool ik_pos_solve(const std::vector<DH>& dh,
  ...
  fk_all(dh, q_io, Tj);
  for (int j=0;j<=6;++j) Tj[j] = T_align * Tj[j];
  return ((target_world - Tj[6].translation()).norm() < opt.pos_tol);
}
    ...
    std::vector<Eigen::Vector3d>* used_samples = nullptr)
{
  ...
  for (size_t i = 0; i < N; ++i) {
    // 1) sample inside current corridor poly
    ...
    for (int rep = 0; rep < 12 && !ok_poly; ++rep) {
      q_poly = q_curr;
      ok_poly = ik_pos_solve(dh, T_align, sample_poly, q_poly, ik_opt);
    ...
    // 2) sample inside intersection of neighboring corridor cells
      ...
      if (H_inter.has_value()) {
        ...
        if (V_inter.size() >= 4 && !SafeIsFlat(V_inter)) {
          ...
          for (int rep = 0; rep < 12 && !ok_inter; ++rep) {
            q_inter = q_curr;
            ok_inter = ik_pos_solve(dh, T_align, sample_inter, q_inter, ik_opt);
...
// Main node
// ============================================================
class SceneSpaceRrtDhIkStatsNode : public rclcpp::Node {
public:
  SceneSpaceRrtDhIkStatsNode() : Node("scene_spacerrt_dh_ik_stats") {
    frame_id_ = declare_parameter<std::string>("frame_id", "world");
    scene_file_ = declare_parameter<std::string>("scene_file", "");
    out_dir_ = declare_parameter<std::string>("out_dir", "/tmp/scene_spacerrt_stats");
    ...
    topic_ = declare_parameter<std::string>("topic", "/scene_spacerrt_stats_markers");
    ...
    if (derive_xyz_from_q_) {
      ...
      for (int j=0; j<=6; ++j) {
        Tj_start[j] = T_align_ * Tj_start[j];
        Tj_goal[j]  = T_align_ * Tj_goal[j];
      }
      start_xyz_ = Tj_start[6].translation();
      goal_xyz_  = Tj_goal[6].translation();
      RCLCPP_INFO(get_logger(), "derive_xyz_from_q=true -> start_xyz=(%.6f, %.6f, %.6f), goal_xyz=(%.6f, %.6f, %.6f)",
...
private:
  ...
  void publish_last_markers_() {
    ...
    for (size_t t=0; t<last_path_q_.size(); ++t) {
      ...
      ma.markers.push_back(MakeSphere(frame_id_, "tip", id++, Tj[6].translation(), 0.012, 0.1, 0.9, 0.9, 0.9));
    }
...
int main(int argc, char** argv) {
  ...
  rclcpp::spin(std::make_shared<SceneSpaceRrtDhIkStatsNode>());
  rclcpp::shutdown();
  return 0;
}
