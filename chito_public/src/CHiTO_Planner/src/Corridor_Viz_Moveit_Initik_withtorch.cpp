...
struct InitIKResult {
  ...
  std::vector<int> anchor_source; // 0=poly, 1=inter
...
class CorridorVizMoveItInitIK : public rclcpp::Node {
public:
  CorridorVizMoveItInitIK(): Node("corridor_viz_moveit_initik") {
  ...
  bool init() {
    ...
    if (joint_cost_weights_.size() != jmg_->getVariableCount()) {
      joint_cost_weights_.assign(jmg_->getVariableCount(), 1.0);
    }
...
private:
  ...
  std::vector<IKCandidate> generateCandidatesForSegment_(const HPolyhedron& sample_poly,
    ...
    std::vector<Eigen::Vector3d> tcp_trials;
    tcp_trials.push_back(first_anchor);
    for (int rs=0; rs<max_resample_per_segment_; ++rs) {
      tcp_trials.push_back(SamplePointIn(sample_poly, rng_, &V_cached));
    }

    const auto* vars = jmg_->getVariableNames().data(); (void)vars;
    for (int t=0; t<(int)tcp_trials.size(); ++t) {
      for (int oi=0; oi<(int)ori_cands_deg.size(); ++oi) {
        for (int k=0; k<ik_seed_trials_per_pose_; ++k) {
          std::vector<double> qtmp;
          bool cfree = false;
          bool ok = solveAtTcp_(tcp_trials[t], ori_cands_deg[oi], qtmp, cfree, T_tip2tcp_local, validity);
          if (ok) {
            IKCandidate c;
            c.q = std::move(qtmp);
            c.tcp_xyz = tcp_trials[t];
  ...
  InitIKResult runInitIK_(const CorridorData& C) {
    ...
    for (size_t i=0;i<N;++i) {
      ...
      auto cand = generateCandidatesForSegment_(sample_poly, R.anchors[i], (int)i,
                                                T_tip2tcp_local, ori_cands_deg, validity, V_poly[i]);
  ...
  bool processOne_(size_t idx) {
    ...
    if (!output_dir_.empty()) {
      ...
      for (size_t i=0;i<C.polys.size();++i) {
        ...
        if (i < C.inters.size()) item["has_intersection_poly"] = true;
        else item["has_intersection_poly"] = false;
  ...
  void drawRobotSkeleton_(MarkerArray& ma, int& id,
    ...
    if (torch_enable_) {
      ...
      ma.markers.push_back(makeCylinder(frame_id_, ns+"_torch1", id++, c1, dir1_world, torch_cyl1_len_, torch_cyl1_dia_,
                        torch1_rgba_[0], torch1_rgba_[1], torch1_rgba_[2], torch1_rgba_[3]));
      const double tilt = -torch_tilt_deg_ * M_PI/180.0;
      Eigen::Matrix3d R_tilt_localX = Eigen::AngleAxisd(tilt, Eigen::Vector3d::UnitX()).toRotationMatrix();
      ...
      ma.markers.push_back(makeCylinder(frame_id_, ns+"_torch2", id++, c2, dir2_world, torch_cyl2_len_, torch_cyl2_dia_,
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<CorridorVizMoveItInitIK>();
  if (!node->init()) {
